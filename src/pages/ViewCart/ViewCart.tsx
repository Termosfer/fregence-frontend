    ...
    <div className="py-10 px-4 sm:px-8 lg:px-20 font-[Playfair] bg-white min-h-screen">
      ...
      <div className="flex flex-col lg:flex-row gap-12">
        <div className="flex-1 overflow-x-auto text-left">
          <table className="w-full border-collapse">
            ...
            <tbody>
              {sortedItems?.map((item: CartItem) => {
                // DÜZƏLİŞ: variantId ilə yoxlayırıq
                  ...
                  >
                    <td className="py-8 table-cell">
                      <div className="flex flex-col sm:flex-row items-start sm:items-center gap-6">
                        ...
                        <div className="space-y-1 text-left">
                          ...
                          {/* YENİ: ML göstəricisi */}
                          <p className="text-[10px] bg-gray-100 w-fit px-2 py-0.5 rounded text-gray-600 font-bold uppercase tracking-wider mb-2">
                             {item.ml} ML
                          </p>
                          ...
                          >
                            ...
                            <span className="relative block text-[10px] font-bold text-red-500 transition-transform duration-300 group-hover:-translate-y-full flex items-center gap-1">
                              <FiTrash2 /> {isThisItemRemoving ? "DELETING..." : "REMOVE"}
                            </span>
                    ...
                    <td className="py-8 block md:table-cell text-left md:text-center">
                      <div className="flex md:justify-center mt-4 md:mt-0">
                        <div className={`flex items-center gap-6 border border-gray-200 px-5 py-2.5 rounded-full bg-white transition-all ${isThisItemUpdating ? "shadow-inner border-gray-300" : "shadow-sm"}`}>
                          <button
                            aria-label="decrease quantity"
                            onClick={() =>
                              item.quantity > 1 &&
                              updateQuantity({
                                variantId: item.variantId, // DÜZƏLİŞ: variantId
                                quantity: -1,
                              })
                            }
                          ...
                          <span className={`w-6 text-center font-bold tabular-nums text-gray-800 ${isThisItemUpdating ? "animate-pulse text-[#81d8d0]" : ""}`}>
                            {item.quantity}
                          </span>
                          ...
                          <button
                            ...
                            className={`cursor-pointer transition-all ${isThisItemUpdating ? "text-gray-200" : "hover:text-black text-gray-500"}`}
                          >
                            <FiPlus />
                          </button>
                        </div>
                    ...
                    <td className="py-8 block md:table-cell text-left md:text-right font-medium text-base text-gray-800 font-[Jost]">
                      <span className="md:hidden font-bold text-neutral mr-2 text-xs uppercase tracking-widest">Total:</span>
                      {item.subTotal}
                      <span className="text-xs"> AZN</span>
                    </td>
                  </tr>
          ...
          <Link to="/products" className="relative inline-block w-fit overflow-hidden group tracking-widest cursor-pointer mt-10 text-black">
            <span className="absolute bottom-0 left-0 h-px *: w-full bg-[#00000080] scale-x-100 group-hover:scale-x-0 transition-transform duration-300"></span>
            <span className="relative block text-xs font-bold text-neutral px-1 transition-transform duration-300 group-hover:-translate-y-full">CONTINUE SHOPPING</span>
            <span className="absolute inset-0 text-xs font-bold text-black px-1 translate-y-full transition-transform duration-300 group-hover:translate-y-0">CONTINUE SHOPPING</span>
          </Link>
        </div>
        ...
        <div className="w-full lg:w-[350px] xl:w-[450px] bg-gray-50 border border-gray-100 rounded-3xl p-10 h-fit shadow-2xl shadow-gray-100/50">
          <h2 className="text-xl font-bold mb-8 uppercase tracking-[2px] text-gray-800 border-b border-gray-200 pb-4">Order Summary</h2>
          <div className="space-y-5 mb-8">
            <div className="flex justify-between text-gray-600 font-medium">
              <span>Subtotal</span>
              <span className="text-black font-medium font-[Jost]">{cartTotal} AZN</span>
            </div>
            <div className="flex justify-between text-gray-600 font-medium">
              <span>Shipping</span>
              <span className="text-green-600 font-bold font-[Jost] text-xs uppercase tracking-widest">Free</span>
            </div>
          </div>
          <div className="flex justify-between font-bold border-t border-gray-200 pt-6 text-2xl text-black">
            <span>Total</span>
            <span className="tracking-tight font-medium font-[Jost]">{cartTotal} AZN</span>
          </div>
          <Link to="/checkout" className="relative inline-flex items-center justify-center w-full h-[60px] bg-black text-white rounded-2xl overflow-hidden group cursor-pointer mt-10 shadow-lg active:scale-[0.98] transition-transform">
            <span className="absolute inset-0 flex items-center justify-center text-xs font-bold tracking-[3px] uppercase transition-transform duration-300 group-hover:-translate-y-full">Proceed to Check Out</span>
            <span className="absolute inset-0 flex items-center justify-center text-xs font-bold tracking-[3px] uppercase translate-y-full transition-transform duration-300 group-hover:translate-y-0">Confirm Order</span>
          </Link>
          <div className="mt-10 text-center">
            <p className="text-[10px] text-neutral font-bold uppercase mb-4 tracking-[2px]">Guaranteed Safe Checkout</p>
            <div className="flex items-center justify-center gap-2">
              <img src={paymentMethods1} alt="Visa" className="h-6" />
              <img src={paymentMethods} alt="Mastercard" className="h-6" />
            </div>
          </div>
        </div>
      </div>
